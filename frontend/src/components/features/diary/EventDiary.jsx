...
const EventDiary = () => {
	...
	const {postEventTraining} = useHttp();

	const onSendEventTraining = (e) => {
		e.preventDefault();

		const formData = new FormData(e.target)
		const errors = diaryValidate(formData, "event")
		
		if (Object.keys(errors).length){
			console.log(errors);
		} else {
			postEventTraining(formData)
